...
std::vector<Texture> Engine::Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {
            std::string file = textures_loaded[j].m_Path.substr(textures_loaded[j].m_Path.find_last_of('/') + 1);
            if (std::strcmp(file.data(), str.C_Str()) == 0)
            {
        ...
        {   // if texture hasn't been loaded already, load it
            ...
            Texture texture(filename, 0, typeName);
